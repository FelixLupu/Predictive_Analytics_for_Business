...
SELECT w.channel, COUNT(w.channel) AS count
FROM web_events w
GROUP BY w.channel
ORDER BY count
...
SELECT a.name, AVG(standard_qty) as avg_std, AVG(gloss_qty) as avg_gloss, AVG (poster_qty) as avg_post
FROM orders o
JOIN accounts a
ON o.account_id = a.id
GROUP BY a.name
...
SELECT a.name, AVG(standard_amt_usd) as avg_std, AVG(gloss_amt_usd) as avg_gloss, AVG (poster_amt_usd) as avg_post
FROM orders o
JOIN accounts a
ON o.account_id = a.id
GROUP BY a.name
...
SELECT account_id, SUM(total_amt_usd) as sum_total
FROM orders
GROUP BY 1
...
/*SUBQUERY*/
SELECT COUNT(*) num_20
FROM (SELECT a.name, COUNT(*) num_orders
      FROM orders o
      JOIN accounts a
      ON o.account_id = a.id
      GROUP by 1
...
SELECT a.name, SUM(total_amt_usd) as sum_total
FROM orders o
JOIN accounts a
ON a.id = o.account_id
GROUP BY 1
...
SELECT DATE_TRUNC ('month', occurred_at), a.name, SUM(gloss_amt_usd)
FROM orders
JOIN accounts a
...
SELECT a.name, SUM(o.total_amt_usd) total_spent, CASE WHEN SUM(o.total_amt_usd) > 200000 THEN 'greater than 200000'
...
SELECT a.name, DATE_PART('year', o.occurred_at) AS year, SUM(o.total_amt_usd) total_spent,
